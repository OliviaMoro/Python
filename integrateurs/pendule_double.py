...
def plotPotentiel():
    ...
    plt.plotSurf(theta1,theta2,V,labelx,labely,'',titre)
    plt.plotContour(theta1,theta2,V,labelx,labely,titre)
...
def testRK4(uCI,dt,n):
    ...
    for i in range(1,n+1):
        j = (i-1)%p
        #(tf,uf)=rk.runge_kutta_4(dt,t[i-1],u[i-1][0],u[i-1][1],u[i-1][2],u[i-1][3],key1=derX1,key2=derY1,key3=derX2,key4=derY2)
        (tf,uf)=rk.runge_kutta_4(dt,tTemp[j],uTemp[j][0],uTemp[j][1],uTemp[j][2],uTemp[j][3],key1=derX1,key2=derY1,key3=derX2,key4=derY2)
        if(i%p == 0):
            # On ajoute les valeurs dans les listes finales
            t.append(tf)
            u.append(uf)
            # On vide les tableaux temporaires
            tTemp[:] = []
            uTemp[:] = []
        tTemp.append(tf)
        uTemp.append(uf)
...
def aniPenduleDouble(dt,n,t,x1,y1,x2,y2):
    ...
    labelx = r'x (m)'
    labely = r'y (m)'
    titre = 'Mouvement du pendule double (m1=m2,l1=l2=0.2 m)'
...
if __name__ == "__main__":  
    ...
    #Premier test de l'algorithme
    dt = 0.005
    n = 2000
    y0 = [0.66,0,0.03,0]
    y1 = [120*math.pi/180.,0,-10*math.pi/180.,0]
    t,x1,y1,x2,y2 = testRK4(y1,dt,n)
    plt.plotGraph(x2,y2,"x2","y2","Variation de y2 selon x2")
    
     #Vérification de l'énergie
    #T,V,E = testEnergie(x1,y1,x2,y2)
    ...
    plt.plotGraph(x_2,v2,"x2","y2","Section de Poincaré (x2,y2)")
